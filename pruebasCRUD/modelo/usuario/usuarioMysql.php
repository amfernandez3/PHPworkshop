...
class usuarioMysql extends usuario {
  ...
  function guardar(){
    $stm = BDMySql::getConexion()->prepare("INSERT into usuario(nombre, correo, rol, password) values(:nombre,:correo,:rol,:password)"); 
            $stm->execute([":nombre"=>$this->getNombre(),":correo"=>$this->getCorreo(),":rol"=>$this->getRol(),":password"=>$this->getPassword()]);
}
...
static function listar(){
    ...
    while (($usuario = $stm->fetch())!=null) {
        $usuarios[] =  new self($usuario["idUsuario"],$usuario["nombre"],$usuario["correo"],$usuario["rol"],$usuario["password"]);
    }
    return $usuarios;
}
...
function modificar($datos){
    ...
    $stm->execute([":nombre"=>$datos->getNombre(),":correo"=>$datos->getCorreo(),":rol"=>$datos->getRol(),":password"=>$datos->getPassword()]);
}
